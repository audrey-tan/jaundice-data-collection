...
export default function FatherDataScreen() {
  let nextRoute = '(screens)';

  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();

  const {state, dispatch} = useContext(AppContext);

  const [selectedIndex, setIndex] = React.useState(state.fSkinTone); 

  function next() {
    dispatch({type: 'change_page', newValue: pageNum+1})
    dispatch({type: 'change_fSkinTone', newValue: selectedIndex})
